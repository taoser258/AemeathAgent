...
> 当前版本 **v0.3.1-alpha**（内测阶段）。到 [Releases](https://github.com/taoser258/AemeathAgent/releases) 下载 Windows 安装包，或按下方步骤从源码运行。
...
- **自动更新**：GitHub Releases 渠道（私有仓库无 token 时静默降级，手动装新包）。
...
├── tests/                     # vitest（只测纯逻辑；695 例）
├── scripts/                   # Office 运行时构建脚本
